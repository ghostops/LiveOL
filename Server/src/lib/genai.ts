...
export async function parseDateWithAI(dateString: string) {
  const response = await ai.models.generateContent({
    ...
    contents: `
      ...
      Input: a single natural-language date/time string that may include a range/span.
      Task: Output exactly one UTC datetime string for the start of the interpreted time (if a span/range), in ISO-8601 format YYYY-MM-DDTHH:MM:SSZ.
      Rules:
        ...
        •	Output only the ISO string (no extra text).
