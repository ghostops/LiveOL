...
export class PlusCodeHandler {
  ...
  public validatePlusCode = async (
    code: string,
    deviceId: string,
  ): Promise<boolean> => {
    await this.fetchPlusCodes();

    return this.claimedCodes.some(
      claimed => claimed.code === code && claimed.deviceId === deviceId,
    );
  };

  public redeemPlusCode = async (
    code: string,
    deviceId: string,
  ): Promise<void> => {
    await this.fetchPlusCodes();

    const codeClaimed = this.claimedCodes.some(
    ...
    const codeToClaim = this.availableCodes.find(
      claimable => claimable.code === code,
    );
    ...
    this.availableCodes = this.availableCodes.filter(
