...
export class LiveresultatAPIClient {
	...
	public getclasses = async (id: number): Promise<LiveresultatApi.getclasses> =>
		this.cachedRequest(this.client.get(`/api.php?method=getclasses&comp=${id}`), `getclasses:${id}`, '1 minute');

	public getclassresults = async (id: number, _class: string): Promise<LiveresultatApi.getclassresults> =>
		this.cachedRequest(
			this.client.get(`/api.php?method=getclassresults&comp=${id}&class=${encodeURIComponent(_class)}`, {
				headers: {
	...
	public getclubresults = async (id: number, club: string): Promise<LiveresultatApi.getclubresults> => {
		return this.cachedRequest(
			this.client.get(`/api.php?method=getclubresults&comp=${id}&club=${encodeURIComponent(club)}`),
