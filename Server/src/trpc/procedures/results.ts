...
export const getResults = publicProcedure
	...
	.query(async ({ ctx, input }) => {
		...
		const res = await ctx.Liveresultat.getclassresults(input.competitionId, input.className);

		const sorted = sortOptimal(res.results, input.sorting || 'place:asc');

		return sorted.map(marshallResult(input.competitionId, input.className, res.splitcontrols));
	});
...
export const getClubResults = publicProcedure
	...
	.query(async ({ ctx, input }) => {
		...
		const res = await ctx.Liveresultat.getclubresults(input.competitionId, input.clubName);

		const sorted = sortOptimal(res.results, input.sorting || 'place:asc');

		return sorted.map((result) => marshallResult(input.competitionId, result.class, [])(result));
	});
